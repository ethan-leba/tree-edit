...
* Getting started
The =evil-tree-edit= can be installed on MELPA.

Many, if not all languages will require custom grammars to be installed.
#+begin_src elisp
(with-demoted-errors ""
  (require 'tree-sitter-langs)
  (tree-sitter-langs-ensure 'te-python "~/Dropbox/tree-sitter-python")
...
It's also recommended to use evil-tree-edit with an autoformatter in it's current
